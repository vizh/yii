...
<?php if ($formNewPartner->hasErrors()): ?>
    <div class="alert alert-error">
        <?= CHtml::errorSummary($formNewPartner) ?>
    </div>
<?php endif ?>

<?php if (!empty($partnerErrorForms)): ?>
    <div class="alert alert-error">
        <?= CHtml::errorSummary($partnerErrorForms) ?>
    </div>
<?php endif ?>

<?php if (!empty($userErrorForms)): ?>
    <div class="alert alert-error">
...
<div class="btn-toolbar">
    <a href="<?= Yii::app()->createUrl('/pay/admin/booking/index'); ?>" class="btn">
...
<div class="well">
    ...
    <h3>
        <?= CHtml::encode($manager->Hotel) ?>,
        <?= CHtml::encode($manager->Housing) ?>,
        №<?= CHtml::encode($manager->Number) ?>
    </h3>
    ...
    <p>
        ...
        Основные места: <em><?= $manager->DescriptionBasic ?></em>; дополнительные места:
        <em><?= $manager->DescriptionMore ?></em><br>
    ...
    <table class="table">
        ...
        <tbody>
        <?= CHtml::beginForm() ?>
        <?php foreach ($partnerBooking as $booking): ?>
            <tr>
                <td>
                    <a href="<?= Yii::app()->createUrl('/pay/admin/booking/partner', ['owner' => $booking->Owner]) ?>"><?= $booking->Owner ?></a>
                </td>
                <?php if (!$booking->Paid): ?>
        ...
        <tr>
            <td colspan="5">
                <?=Html::errorSummary($formNewPartner)?>
            </td>
        </tr>
        ...
        <tr>
            ...
            <td>
                <button class="btn" type="submit" name="createPartner" value="1"><i class="icon-plus"></i></button>
            </td>
        </tr>
        <?= CHtml::endForm() ?>
        </tbody>
    ...
    <table class="table">
        ...
        <?php foreach ($orderItems as $item): ?>
            <tr>
                ...
                <?php if (!$item->Paid): ?>
                    ...
                    <td>
                        <a class="btn btn-danger booking-delete"
                           href="<?= Yii::app()->createUrl('/pay/admin/booking/delete', ['type' => 'user', 'id' => $item->Id]) ?>">
        ...
        <?= CHtml::endForm() ?>
            ...
            <?= CHtml::beginForm() ?>
                <tr>
                    <td colspan="5">
                        <?=Html::errorSummary($formNewUser)?>
                    </td>
                </tr>
                <tr>
                    <td>
                        <?php $this->widget('application\widgets\AutocompleteInput', [
                            'name' => 'userNewData[RunetId]',
