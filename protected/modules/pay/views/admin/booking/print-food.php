    ...
    <table class="order" cellpadding="0" cellspacing="0" style="font-size: 12px; width: 100%; padding: 0; margin: 10px 0 0 10px; position: relative; left: -50px; border-collapse: collapse;">
        <thead>
            <?foreach (PartnerFoodOrder::getFoodProductData() as $date => $list):?>
                <th colspan="<?=sizeof($list);?>" style="padding: 30px 8px;text-align: center;font-weight: normal;background-color: #f2f2f2;border: 1px solid #000000;"><?=\Yii::app()->getDateFormatter()->format('dd.MM.yyyy', $date);?></th>
            <?endforeach;?>
            <th style="padding: 30px 8px;text-align: center;font-weight: normal;background-color: #f2f2f2;border: 1px solid #000000;">ИТОГО, в т.ч.  НДС 18%</td>
        </thead>
        <tbody>
        <tr>
            <?foreach (PartnerFoodOrder::getFoodProductData() as $date => $list):?>
