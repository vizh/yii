    ...
    private function initParticipants()
    {
        ...
        foreach ($result as $row) {
            ...
            if ($user !== null) {
                ...
                $item->RunetId = $row['ownerRunetId'];

                $participant = Participant::model()->byUserId($user->Id)
                    ->byEventId(\Yii::app()->params['AdminBookingEventId'])->find();
                if ($participant == null) {
                    continue;
                }
                ...
                if ($participant->RoleId == 3) {
                    ...
                    $orderItem = OrderItem::model()->byEventId(\Yii::app()->params['AdminBookingEventId'])->byPaid(true)
                        ->byAnyOwnerId($user->Id)->find($criteria);
                    if ($orderItem == null) {
                        continue;
                    }
                    ...
                    $item->Dates = $this->getDateList($orderItem->getItemAttribute('DateIn'),
                        $orderItem->getItemAttribute('DateOut'));
                }
                $this->parking[] = $item;
            }
        }
