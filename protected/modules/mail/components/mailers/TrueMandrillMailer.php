 ...
 */
class TrueMandrillMailer extends \mail\components\Mailer
{
    ...
    public function internalSend($mails)
    {
        ...
        foreach ($mails[0]->getAttachments() as $name => $path) {
            $attachments[] = [
                'name' => $name,
                'type' => is_array($path) ? $path[0] : \CFileHelper::getMimeType($path),
                'content' => base64_encode(
                    file_get_contents(is_array($path) ? $path[1] : $path)
                )
            ];
        }
