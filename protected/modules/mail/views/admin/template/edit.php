...
<?php $activeForm = $this->beginWidget('\application\widgets\ActiveForm', ['htmlOptions' => ['enctype' => 'multipart/form-data', 'class' => 'form-horizontal']]);?>
    ...
    <div class="well">
        ...
        <div class="control-group">
            ...
            <div class="controls">
                <?php if (!$form->isUpdateMode() || !$template->checkViewExternalChanges()):?>
                    <?=$activeForm->textArea($form, 'Body', ['class' => 'input-block-level', 'style' => 'height: 500px;']);?>
                    <p class="m-top_10 muted">
                        <?php foreach ($form->bodyFieldLabels() as $field => $label): ?>
        ...
        <h3>Выборка</h3>
        <?php foreach ($form->getConditionData() as $key => $value): ?>
