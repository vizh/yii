...
use application\models\RequiredStatus;
use Yii;
...
class Account extends \CFormModel
{
    ...
    {
        return array_merge(['' => 'Выберите тип аккаунта'], AccountModel::getRoleLabels());
    }

    public function getRequestPhoneOnRegistrationStatusData()
    {
        return [
            RequiredStatus::None => Yii::t('app', 'Нет'),
            RequiredStatus::Required => Yii::t('app', 'Да, обязательный ввод'),
            RequiredStatus::NotRequired => Yii::t('app', 'Да, но не обязательно')
        ];
    }
