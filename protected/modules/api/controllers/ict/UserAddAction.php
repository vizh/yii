...
class UserAddAction extends Action
{
     ...
     *     description="Добавляет пользователя ICT",
     *     request=@Request(
     *          method="GET",
     *          url="/ict/useradd",
     *          params={
     *              @Param(title="RunetId", mandatory="Y", description="RunetId пользователя"),
     *              @Param(title="RoleId", mandatory="Y", description="Роль пользователя ICT. 1 -  Лидер. 2 - Эксперт. 3 - участник."),
    ...
    public function run()
    {
        ...
        if ($ictUser === null) {
            $ictUser = new IctUser();
            $ictUser->UserId = $user->Id;
            $ictUser->RoleId = $role->Id;
            if ($profInterest !== null) {
                $ictUser->ProfessionalInterestId = $profInterest->Id;
            }
            $ictUser->Type = $type;
        } else {
            if (!empty($ictUser->ExitTime)) {
                $ictUser->ExitTime = null;
            } else {
