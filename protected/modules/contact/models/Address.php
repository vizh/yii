...
class Address extends ActiveRecord
{
    ...
    const DEFAULT_MAP_SCALE = 12; //масштаб карты, по-умолчанию
    ...
    public function afterFind()
    {
        if (empty($this->GeoPoint)) {
            $GeoPoint = [0 => '', 1 => '', 2 => self::DEFAULT_MAP_SCALE];
        } else {
            $GeoPoint = explode(',', $this->GeoPoint);
        }

        if (!isset($GeoPoint[2])) {
            $GeoPoint[2] = self::DEFAULT_MAP_SCALE;
        }
    ...
    /**
     * Масштаб карты, для билета
     * @return mixed
     */
    public function getMapScale()
    {
        if (!isset($this->getGeoPointCoordinates()[2])) {
            return self::DEFAULT_MAP_SCALE;
        }
        return $this->getGeoPointCoordinates()[2];
    }
    ...
    {
        $geocode = $this->__toString();

        $coordinates = self::defineGeoPointCoordinates($geocode);

        if (!empty($coordinates)) {
            $this->GeoPoint = $this->setGeoPointCoordinatesManual($coordinates[1], $coordinates[0], self::DEFAULT_MAP_SCALE);
        }
    }
     ...
     * @return array
     */
    public static function defineGeoPointCoordinates($geocode)
    {
        $coordinates = [];
        ...
        curl_setopt($curl, CURLOPT_URL, 'http://geocode-maps.yandex.ru/1.x/?' . http_build_query(['format' => 'json', 'geocode' => $geocode]));
     ...
     * @param $longitude
     */
    public function setGeoPointCoordinatesManual($latitude, $longitude, $scale)
    {
        $this->GeoPoint = [$latitude, $longitude, $scale];
    }
}