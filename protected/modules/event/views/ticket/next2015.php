...
foreach ($participant as $part) {
    ...
    $criteria->params['Title'] = $part->Part->Title;
    $section = Section::model()->byEventId($event->Id)->find($criteria);
    if ($section == null) {
        continue;
    }
    $date = \Yii::app()->getDateFormatter()->format('dd MMMM yyyy', $section->StartTime);
    if (isset($statuses[$date])) {
        if ($part->Role->Priority > $statuses[$date]->Priority) {
            $statuses[$date] = $part->Role;
        }
    } else {
        $statuses[$date] = $part->Role;
    }
        ...
        <tbody>
            ...
            <tr>
                <td style="height: 20mm; vertical-align: bottom; font-size: 3mm;">
                    <?php foreach ($statuses as $date => $role):?>
                        <span style="text-transform: uppercase;"><?=$date?>:</span> <?=$role->Title;?><br/>
                    <?php endforeach;?>
                </td>
            </tr>
        </tbody>
