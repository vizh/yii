...
use \event\components\handlers\Create;
        ...
        if ($request->getIsPostRequest() && $form->validate(null,false))
        {
            ...
            if ($event->save())
            {
                ...
                if (!empty($form->Url)) {
                    ...
                    $event->setContactSite($url, (isset($parseUrl['scheme']) && $parseUrl['scheme'] == 'https' ? true : false));
                }
                ...
                $mail = new Create($mailer, $form, $event);
                $mail->send();

                $mail2 = new Create($mailer, $form, $event);
