...
class Edit extends \CFormModel
{
    ...
    public function filterEmails($emails)
    {
        ...
        if (!$valid) {
            $this->addError('Emails', \Yii::t('app', 'Ошибка в заполнении поля {attribute}.', ['{attribute}' => $this->getAttributeLabel('Emails')]));
        }
        return $emails;
    }
    ...
    public function filterFullInfo($value)
    {
        ...
        $purifier->options = [
            'HTML.AllowedElements' => ['a', 'strong', 'em', 's', 'ol', 'li', 'ul', 'p'],
            'HTML.AllowedAttributes' => ['href', 'name', 'id'],
            'Attr.EnableID' => true
        ];
