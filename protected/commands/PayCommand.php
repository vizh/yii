...
class PayCommand extends BaseConsoleCommand
{
    ...
    /**
     * Удаление забронированных номеров для рифа15, если это физический счет - заказ удаляется, если счет юр. - удаляется счет или заказ из счета
     * @throws CDbException
     */
    public function actionClearRif15Book()
    {
        $orderItems = OrderItem::model()->byEventId(\Yii::app()->params['AdminBookingEventId'])->byPaid(false)->byBooked(false)->byDeleted(false)->findAll();
        foreach ($orderItems as $orderItem) {
            $orderLinks = $orderItem->OrderLinks(['with' => ['Order']]);
            /** @var \pay\models\OrderLinkOrderItem $orderLink */
