...
class RoomProductManager extends BaseProductManager
{
  ...
  public function CheckProduct($user, $params = array())
  {
    ...
    $checkRole = EventRoles::GetById(1);
    if (!empty($eventUser) && $eventUser->EventRole->Priority >= $checkRole->Priority)
    {
      return true;
    }

    //$orderItems = OrderItem::GetByEventId($user->UserId, $this->product->EventId);
    $orderItems = OrderItem::GetAllByEventId($this->product->EventId, $user->UserId, $user->UserId);
  ...
  public function Filter($params, $filter)
  {
    ...
    else
    {
      ...
      $model = Product::model()->with(array('Attributes' => array('on' => $filterSql), 'Prices'));
  ...
  public function GetFilterProduct($params)
  {
    ...
    $model = Product::model()->with(array('Attributes', 'Prices'));
    $criteria = new CDbCriteria();
    $criteria->addInCondition('t.ProductId', $productIdList);
    $criteria->limit = 1;
