...
<div class="question <?=!empty($dependIdList) ? 'depend' : '';?>" data-id="<?=$question->QuestionId;?>" data-depends="<?=CHtml::encode(json_encode($dependIdList));?>">
  ...
  <ul>
    ...
    <li>
      <input name="Questions[<?=$question->QuestionId;?>][<?=$answer->AnswerId?>]" type="text" value="<?=isset($this->Values[$answer->AnswerId]) ? $this->Values[$answer->AnswerId] : '';?>" autocomplete="off">
    </li>
    <?endforeach;?>
  </ul>
