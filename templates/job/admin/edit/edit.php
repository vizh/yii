<section class="main edit vacancyedit">
  ...
  <form id="form_editjob" enctype="multipart/form-data" action="" method="post">
    <input type="hidden" value="<?=$this->VacancyId;?>" name="vacancy_id">
    ...
    <aside class="col-l">
      ...
      <div class="aftertitle">
        ...
        <input style="width:480px;" autocomplete="off" class="bordered" type="text" maxlength="250" name="data[email]" value="<?=$this->Email;?>">
      </div>
      ...
      <div class="main-block bordered">
        ...
        <div class="general-container">
          <ul class="inputs-list">
            ...
            <li>
              <label>
                <input type="radio" value="<?=$schedule?>" name="data[schedule]"
                  <?=$schedule==$this->Schedule ? 'checked="checked"' : '';?>>
                <?=$this->words['schedule'][$schedule];?>
              </label>
            </li>
          <?endforeach;?>
          </ul>
      ...
      <div class="main-block bordered">
        <h4>Профессиональный тест</h4>
        <div class="general-container">
          <?if (empty($this->JobTest)):?>
          <a href="<?=RouteRegistry::GetAdminUrl('job', 'test', 'new', array('id' => $this->VacancyId));?>" class="button big">Создать тест</a>
          <?else:?>
          <!--<strong>Количество вопросов:</strong>&nbsp;10<br>
          <strong>Активных вопросов:</strong>&nbsp;5<br>
          <strong>Проходной балл/Максимальный балл:</strong>&nbsp;20/25<br>
          <strong>Время на прохождение:</strong>&nbsp;10&nbsp;минут<br>
          <strong>Повторное прохождение:</strong>&nbsp;3&nbsp;часа<br>-->
          <a href="<?=RouteRegistry::GetAdminUrl('job', 'test', 'edit', array('id' => $this->JobTest->TestId));?>" class="button big">Редактировать тест</a>
          <?endif;?>
        </div>
      </div>
    </aside>
    ...
    <aside class="col-r">
      <div class="pub bordered sidebar">
        ...
        <a target="_blank"
           href="<?=RouteRegistry::GetUrl('job', '', 'show', array('id' => $this->VacancyId));?>"
      ...
      <div class="sidebar bordered">
        ...
        <div class="sub-element">
          <h5>Страна</h5>
          <?=GeoViewer::GetCountriesSelect($this->CountryId, 'data[country]', 'bordered');?>
        </div>
        <div class="sub-element">
          <h5>Регион</h5>
          <?=GeoViewer::GetRegionSelect($this->CountryId, $this->RegionId, 'data[region]', 'bordered');?>
        </div>
        <div class="sub-element">
          <h5>Город</h5>
          <?=GeoViewer::GetCitySelect($this->RegionId, $this->CityId, 'data[city]', 'bordered');?>
        </div>
      </div>

      <div class="sidebar bordered selectcompany">
        <h4>Выбор компании</h4>
        <div class="sub-element">
          <h5>Название компании</h5>
          <input type="hidden" name="data[companyId]" value="<?=$this->CompanyId;?>">

          <input id="selectcompany" class="bordered" type="text" name="data[companyName]" value="<?=$this->CompanyName;?>" autocomplete="off">
          <?if (empty($this->CompanyId)):?>
            <br><br><span style="color:#d55;"></span>
          <?endif;?>
        </div>
      </div>
