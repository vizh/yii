...
class UtilityMail extends AdminCommand
{
  ...
  public static $MailName = 'siteconf12-3';
...
//    $criteria->addInCondition('EventUsers.EventId', array(95,43,9,171,107,200,248,196));
    ...
    $criteria->addInCondition('EventUsers.EventId', array(246));
...
//    );
    ...
    //$criteria->addInCondition ('t.RocId', array(35287, 454));//, 49465));

//    $criteria->addInCondition('t.RocId', array(35287, 337, 12959, 454, 12953, 77777, 38876, 18120));

    //$criteria->addInCondition ('t.RocId', $iResearch3);
...
//    $this->mail->RoleName = $user->EventUsers[0]->EventRole->Name;
    ...
    $mail->SetFrom('info@siteconf.ru', 'ROCID:// Календарь', false);
    $mail->CharSet = 'utf-8';
    $mail->Subject = '=?UTF-8?B?'. base64_encode('САЙТ 2012: успейте оплатить до 31 июля!') .'?=';
    $mail->Body = $this->mail;
...
//    $secret = 'gQVcymFs5NkY0jjNOxuRcfkKC'; // rocid.ru
    $secret = 'vyeavbdanfivabfdeypwgruqe'; // sp-ic.ru

    $timestamp = time();
    $rocid = $user->RocId;
    $checkKey = substr(md5($rocid.$secret), 0, 16);

    $secret = 'gQVcymFs5NkY0jjNOxuRcfkKC';  //i-research
...
//    return 'http://sp-ic.ru/my/?ROCID=' . $rocid . '&KEY=' . $checkKey;
//    return 'http://2012.russianinternetforum.ru/my/?ROCID=' . $rocid . '&KEY=' . $checkKey .'&FOODPAGE';

    return 'http://siteconf.ru/my/?ROCID=' . $rocid . '&KEY=' . $checkKey;
