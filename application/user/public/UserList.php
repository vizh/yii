...
class UserList extends GeneralCommand
{
  protected function preExecute()
  {
    ...
    $this->SetTitle($titles['user_list']);
  }  
  ...
  protected function doExecute($letter = '', $location = '0-0-0', $page = '1')
  {
    ...
    $userList = User::GetUserListByLocation($letter, $location, intval($page));
    $users = $userList['users'];
    $count = $userList['count'];
    
    $jsCode = $this->getJsCode($letter, $location);
    $this->view->HeadScript(array('script' => $jsCode));
    $this->view->Location = $location['country'] == 0 ? '' : $location['country'] . '-' . $location['region'] . '-' . $location['city'] . '/'; 
    ...
    $this->view->SearchWord = $this->getSearchWord($location);
  ...
  private function getJsCode($letter, $location)
  {
    ...
    {
      $view->DropCountry = true;
    }
    else if ($location['region'] == 0)
    {
      $view->DropRegion = true;
    }
    else if ($location['city'] == 0)
    {
      $view->DropRegion = true;
    }
  ...
  private function getSearchWord($location)
  {
    $list = Registry::GetWord('list');
    if ($location['country'] == 0)
    {
      return $list['firstsearch'];
    }
    else
    {
      return $list['othersearch'];
    }
  }
  ...
  private function getPaginator($letter, $location, $page, $count)
  {
    ...
    $url = '/user/list/' . $letter . '/' . $location['country'] . '-' . $location['region'] . '-' . $location['city'] . '/%s/';
