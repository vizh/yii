...
class CIpValidator extends CValidator
{
    ...
    public function __construct()
    {
        if (!$this->ipv4 && !$this->ipv6) {
            throw new Exception('Both IPv4 and IPv6 checks can not be disabled at the same time');
        }
    ...
    private function validateSubnet($ip)
    {
        ...
        if ($this->getIpVersion($ip) === CIpHelper::IPV6) {
            if ($cidr !== null) {
                if ($cidr > CIpHelper::IPV6_ADDRESS_LENGTH || $cidr < 0) {
                    return [$this->wrongCidr, []];
                }
            } else {
                $isCidrDefault = true;
                $cidr = CIpHelper::IPV6_ADDRESS_LENGTH;
            }
            ...
            if ($cidr !== null) {
                if ($cidr > CIpHelper::IPV4_ADDRESS_LENGTH || $cidr < 0) {
                    return [$this->wrongCidr, []];
                }
            } else {
                $isCidrDefault = true;
                $cidr = CIpHelper::IPV4_ADDRESS_LENGTH;
            }
    ...
    {
        return CIpHelper::expandIPv6($ip);
    }
    ...
    {
        return CIpHelper::getIpVersion($ip);
    }
    ...
    {
        return CIpHelper::inRange($ip . '/' . $cidr, $range);
    }
    ...
    {
        $message = $this->message !== null ? $this->message : Yii::t('yii', '{attribute} is not a valid IP address.');
        $message = strtr($message, array(
            '{attribute}' => $model->getAttributeLabel($attribute),
        ));

        $js = "if(!value.match($this->ipv4Pattern) && !value.match($this->ipv6Pattern)){messages.push(" . CJSON::encode($message) . ");}";
